...
func TestNewWithEndpoint(t *testing.T) {
	config, err := NewWithEndpoint("AccessKeyID", "SecretAccessKey", "test.shanhe.com:444/iaas?a=1#hhh")
	assert.NotNil(t, err)

	config, err = NewWithEndpoint("AccessKeyID", "SecretAccessKey", "http:test.shanhe.com:444/iaas?a=1#hhh")
	assert.NotNil(t, err)

	config, err = NewWithEndpoint("AccessKeyID", "SecretAccessKey", "https://api.shanhe.com/iaas")
	assert.Nil(t, err)
	assert.Equal(t, "AccessKeyID", config.AccessKeyID)
	assert.Equal(t, "SecretAccessKey", config.SecretAccessKey)
	assert.Equal(t, "https", config.Protocol)
	assert.Equal(t, "api.shanhe.com", config.Host)
	assert.Equal(t, 443, config.Port)
	assert.Equal(t, "/iaas", config.URI)

	config, err = NewWithEndpoint("AccessKeyID", "SecretAccessKey", "http://test.shanhe.com:444/iaas")
	assert.Nil(t, err)
	assert.Equal(t, "AccessKeyID", config.AccessKeyID)
	assert.Equal(t, "SecretAccessKey", config.SecretAccessKey)
	assert.Equal(t, "http", config.Protocol)
	assert.Equal(t, "test.shanhe.com", config.Host)
